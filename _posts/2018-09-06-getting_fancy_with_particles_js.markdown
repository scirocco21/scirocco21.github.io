...
Integrating particles.js into the asset pipeline is only slightly more involved than getting started with Bootstrap. However, while it is enough to include a link to the external Bootstrap files in a script tag of one's index.html, for example, with particles.js it is easier to just download and add the relevant files to assets/javascripts (in Rails). The relevant files are particles.min.js and particles.json. The json file is the one that ultimately carries all the configurations (how many particles, what shape, etc.):
...
[Create Particle Effects With Particles.js](https://www.youtube.com/watch?v=qK3cgD09Qf0&t=879s)
